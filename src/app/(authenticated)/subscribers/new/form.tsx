...
export default function NewForm() {
  ...
  const Field = (p: any) => (
    <div><label className="block text-xs font-medium text-gray-400 mb-1.5">{p.label}{p.req && " *"}</label><input value={(form as any)[p.k]} onChange={e => setForm({ ...form, [p.k]: e.target.value })} placeholder={p.ph} required={p.req} className="input-modern" /></div>
  );

  return (
    <form onSubmit={submit} className="glass rounded-2xl p-6 space-y-4">
      {err && <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-300 text-sm flex gap-2"><AlertCircle className="w-4 h-4" />{err}</div>}
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div><label className="block text-xs font-medium text-gray-400 mb-1.5">Categoría</label><select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} className="select-modern"><option>DOMICILIAR</option><option>COMERCIAL</option><option>INDUSTRIAL</option><option>PUBLICO</option></select></div>
        ...
        <Field label="Teléfono ASADA ref" k="telefono" ph="" />
      </div>
